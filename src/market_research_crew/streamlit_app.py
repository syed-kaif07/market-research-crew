...
Changes from original:
  - stdout=None, stderr=None in Popen so CrewAI verbose output + colored
    agent banners from main.py stream live to the IDE terminal
  - Added terminal hint banner in UI while running
...
.hero-sub { font-size: 1.1rem; color: #6B7280; margin-top: 8px; font-weight: 300; }
.hero-badge {
...
[data-testid="stTextInput"] input {
    ...
    font-size: 1rem !important; padding: 16px 20px !important;
}
...
[data-testid="stButton"] > button {
    background: #00FF88 !important; color: #080B0F !important;
    border: none !important; border-radius: 12px !important;
    font-family: 'Space Mono', monospace !important; font-weight: 700 !important;
    font-size: 0.9rem !important; padding: 16px 32px !important;
...
[data-testid="stButton"] > button:disabled {
    background: #1C2128 !important; color: #4B5563 !important; cursor: not-allowed !important;
}
.agent-card {
...
.success-msg {
    ...
    font-family: 'Space Mono', monospace; font-size: 0.85rem; margin: 16px 0;
}
.terminal-hint {
    ...
    font-size: 0.75rem; color: #6B7280; margin: 12px 0;
}
.terminal-hint span { color: #00FF88; }
.output-box {
    background: #0D1117; border: 1px solid #1C2128; border-radius: 12px;
    padding: 24px; font-size: 0.9rem; line-height: 1.7;
...
def start_crew_process(product_idea: str) -> subprocess.Popen | None:
    """
    Spawn the CrewAI subprocess.

    KEY CHANGE: stdout=None, stderr=None
    ──────────────────────────────────────
    The child process inherits the parent's terminal handles directly.
    This means every print() in main.py — including the colored agent
    banners AND CrewAI's own verbose output — streams live to the IDE
    terminal in real time while the user watches Streamlit update the UI.

    Original used stdout=log_fh which silently swallowed all output into
    a file, making the terminal blank during the run.
    """
    ...
    try:
        proc = subprocess.Popen(
            ...
            stdout=None,              # ← streams live to IDE terminal
            stderr=None,              # ← errors visible in IDE terminal
            close_fds=(sys.platform != "win32"),
        )
        logger.info("Started crew process pid=%d for product_idea=%r", proc.pid, product_idea)
        return proc
    except OSError as exc:
...
with col1:
    ...
    # Terminal hint — shown only while crew is running
    if st.session_state.running:
        st.markdown(
            '<div class="terminal-hint">'
            '👀 Check your <span>IDE terminal</span> — '
            'live agent output is streaming there right now!'
            '</div>',
            unsafe_allow_html=True,
        )
    ...
    if st.session_state.running:
        ...
        if num_done == len(AGENTS):
            ...
            st.error(
                ...
                "Check your IDE terminal for the full error output."
            )
            ...
            mins = int(elapsed // 60)
            secs = int(elapsed % 60)
            st.info(f"⏱️ Running… {mins}m {secs}s elapsed  |  Watch your IDE terminal for live agent output!")
